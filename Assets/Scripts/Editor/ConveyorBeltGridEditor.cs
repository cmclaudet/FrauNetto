...
public class ConveyorBeltGridEditor : Editor
{
    void OnSceneGUI()
    {
        ...
        if (grid.gridSize.x <= 0 || grid.gridSize.y <= 0 || grid.cellSize <= 0)
            return;
        ...
        {
            float worldX = -y * grid.cellSize;
            float startZ = -grid.gridSize.x * 0.5f * grid.cellSize;
            float endZ = grid.gridSize.x * 0.5f * grid.cellSize;
        ...
        {
            float worldZ = (x - grid.gridSize.x * 0.5f) * grid.cellSize;
            float startX = 0;
            float endX = -grid.gridSize.y * grid.cellSize;
        ...
        // Bottom left (0, 0)
        Vector3 bottomLeft = transform.TransformPoint(new Vector3(0, 0, -grid.gridSize.x * 0.5f * grid.cellSize));
        Handles.Label(bottomLeft, "(0, 0)");

        // Bottom right (gridSize.x-1, 0)
        Vector3 bottomRight = transform.TransformPoint(new Vector3(0, 0, grid.gridSize.x * 0.5f * grid.cellSize));
        Handles.Label(bottomRight, $"({grid.gridSize.x-1}, 0)");

        // Top left (0, gridSize.y-1)
        Vector3 topLeft = transform.TransformPoint(new Vector3(-grid.gridSize.y * grid.cellSize, 0, -grid.gridSize.x * 0.5f * grid.cellSize));
        Handles.Label(topLeft, $"(0, {grid.gridSize.y-1})");

        // Top right (gridSize.x-1, gridSize.y-1)
        Vector3 topRight = transform.TransformPoint(new Vector3(-grid.gridSize.y * grid.cellSize, 0, grid.gridSize.x * 0.5f * grid.cellSize));
